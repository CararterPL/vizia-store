...
// Komponent wewnętrzny z całą logiką
function CheckoutContent() {
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      ...
      if (data.url) {
        window.location.href = data.url; // Przekierowanie do Stripe lub strony sukcesu
      } else {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7 space-y-12">
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <input placeholder="Imię" value={buyerData.firstName} onChange={(e)=>setBuyerData({...buyerData, firstName: e.target.value})} className={`bg-zinc-900 border ${errors.firstName ? `border-${viziaRed}` : 'border-white/10'} p-4 text-sm outline-none focus:border-white transition-colors`} />
                  {errors.firstName && <span className={`text-[10px] text-${viziaRed}`}>{errors.firstName}</span>}
                </div>
                <div className="flex flex-col gap-1">
                  <input placeholder="Nazwisko" value={buyerData.lastName} onChange={(e)=>setBuyerData({...buyerData, lastName: e.target.value})} className={`bg-zinc-900 border ${errors.lastName ? `border-${viziaRed}` : 'border-white/10'} p-4 text-sm outline-none focus:border-white transition-colors`} />
                  {errors.lastName && <span className={`text-[10px] text-${viziaRed}`}>{errors.lastName}</span>}
                </div>
                <div className="col-span-2 flex flex-col gap-1">
                  <input placeholder="E-mail" value={buyerData.email} onChange={(e)=>setBuyerData({...buyerData, email: e.target.value})} className={`bg-zinc-900 border ${errors.email ? `border-${viziaRed}` : 'border-white/10'} p-4 text-sm outline-none focus:border-white transition-colors`} />
                  {errors.email && <span className={`text-[10px] text-${viziaRed}`}>{errors.email}</span>}
                </div>
                <div className="col-span-2 flex flex-col gap-1">
                  <input placeholder="Numer telefonu" value={buyerData.phone} onChange={(e)=>setBuyerData({...buyerData, phone: e.target.value})} className={`bg-zinc-900 border ${errors.phone ? `border-${viziaRed}` : 'border-white/10'} p-4 text-sm outline-none focus:border-white transition-colors`} />
            ...
            <section className="space-y-6">
              <div className="flex justify-between items-center border-l-4 border-white pl-4">
                ...
                <label className={`flex items-center gap-2 cursor-pointer text-xs text-white/70 hover:text-${viziaRed} transition-colors`}>
                  <input type="checkbox" checked={invoiceData.sameAsBuyer} onChange={()=>setInvoiceData({...invoiceData, sameAsBuyer: !invoiceData.sameAsBuyer})} />
                  Takie same jak nabywcy
                </label>
              </div>
              {!invoiceData.sameAsBuyer && (
                <div className="grid grid-cols-2 gap-4">
                  <input placeholder="Nazwa firmy" value={invoiceData.companyName} onChange={(e)=>setInvoiceData({...invoiceData, companyName: e.target.value})} className="col-span-2 bg-zinc-900 border border-white/10 p-4 text-sm outline-none focus:border-white" />
                  <input placeholder="NIP" value={invoiceData.nip} onChange={(e)=>setInvoiceData({...invoiceData, nip: e.target.value})} className="col-span-2 bg-zinc-900 border border-white/10 p-4 text-sm outline-none focus:border-white" />
                </div>
              )}
            </section>
            ...
            <section className="space-y-6">
              ...
              {deliveryMethod === 'kurier' ? (
                <div className="grid grid-cols-2 gap-4">
                  <input placeholder="Ulica i numer" value={shippingAddress.street} onChange={(e)=>setShippingAddress({...shippingAddress, street: e.target.value})} className="bg-zinc-900 border border-white/10 p-4 text-sm outline-none focus:border-white" />
                  <input placeholder="Kod pocztowy" value={shippingAddress.zipCode} onChange={(e)=>setShippingAddress({...shippingAddress, zipCode: e.target.value})} className="bg-zinc-900 border border-white/10 p-4 text-sm outline-none focus:border-white" />
                  <input placeholder="Miasto" value={shippingAddress.city} onChange={(e)=>setShippingAddress({...shippingAddress, city: e.target.value})} className="bg-zinc-900 border border-white/10 p-4 text-sm outline-none col-span-2 focus:border-white" />
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="h-[350px] overflow-y-auto border border-white/10 bg-black p-2 space-y-2">
                      {loadingPoints ? <p className="text-xs p-4 animate-pulse">Szukam punktów...</p> : 
                        points.map((p) => (
          ...
          <div className="lg:col-span-5">
            <div className="bg-zinc-900 p-8 border border-white/10 sticky top-[100px]">
              ...
              <button onClick={handleCompleteOrder} disabled={loading} className={`w-full py-6 mt-10 text-xl font-bold transition-all ${loading ? 'bg-zinc-800 text-zinc-500 cursor-wait' : `bg-white text-black hover:bg-${viziaRed} hover:text-white`}`}>
